...
class CongViecModel extends Model
{
    ...
    public function SELECT_CONG_VIEC_PHANCONG($P_TIME_START, $P_TIME_END, $P_ID_ND, $P_ID_DA,$P_ID_DU_AN_KH)
    {
        $pdo = DB::getPdo();
        $stmt = $pdo->prepare("SELECT SELECT_CONG_VIEC_PHANCONG('$P_TIME_START', '$P_TIME_END', '$P_ID_ND', $P_ID_DA, $P_ID_DU_AN_KH, $P_ID_LOAI_CV) FROM dual");
        $result = $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
    }

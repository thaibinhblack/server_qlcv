...
class DuAnKHModel extends Model
{
    ...
    public function DELETE_CONG_VIEC_DAKH($P_ID_CV_DA, $P_ID_DU_AN_KH)
    {
        $sql = "DECLARE
        P_ID_CV_DA NUMBER(10);
        P_ID_DU_AN_KH NUMBER(10);
    BEGIN
        :n := DELETE_CONG_VIEC_DAKH(:P_ID_CV_DA, :P_ID_DU_AN_KH);
    END;";  
    $pdo = DB::getPdo();
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':P_ID_CV_DA',$P_ID_CV_DA);
    $stmt->bindParam(':P_ID_DU_AN_KH',$arr_params["P_ID_DU_AN"]);
    $stmt->bindParam(':n',$result);
    $stmt->execute();
    return $result;
    }

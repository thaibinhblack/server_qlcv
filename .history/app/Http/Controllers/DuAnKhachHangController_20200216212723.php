    ...
    public function store(Request $request)
    {
        ...
        if($request->has('P_TEN_DU_AN_KH') && $request->has('P_ID_DU_AN') && $request->has('P_TRANG_THAI_DU_AN'))
        {
            if($request->has('api_token'))
            {
                ...
                $result = $du_an_kh_model->THEM_UPDATE_DU_AN_KH($P_ID_DU_AN_KH, $P_ID_DU_AN, $P_TEN_DU_AN_KH, $P_MO_TA_DU_AN, $P_GHI_CHU_DU_AN, $P_TRANG_THAI_DU_AN, $P_ID_KHACH_HANG, $P_ACTION);
                if($result)
                {

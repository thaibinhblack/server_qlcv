...
class DuAnKhachHangController extends Controller
{
    ...
    public function CallFunctionTrangThai($P_ID_TRANG_THAI, $P_TEN_TRANG_THAI, $P_NOI_DUNG_TRANG_THAI, $P_GHI_CHU_TRANG_THAI, $P_ACTION)
    {
        $sql = "DECLARE
            ...
            P_ACTION NUMBER(1);
        BEGIN
            :n := THEM_CAPNHAT_TT_DUAN_KH(:P_ID_TRANG_THAI, :P_TEN_TRANG_THAI, :P_TEN_DU_AN_KH, :P_NOI_DUNG_TRANG_THAI, :P_GHI_CHU_TRANG_THAI,:P_ACTION);
        END;";  
        $pdo = DB::getPdo();
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':P_ID_DU_AN_KH',$P_ID_DU_AN_KH);
        ...
        if($request->has('api_token'))
        {
           ...
           if($request->has('id_du_an'))
           {
            ...
            $du_an = DB::SELECT("SELECT DA_KH.*, DA.TEN_DU_AN, KH.TEN_KH, KH.ID_KHACH_HANG FROM TB_DU_AN_KH DA_KH, TB_DU_AN DA, TB_KH KH
                WHERE DA_KH.ID_DU_AN = DA.ID_DU_AN AND DA_KH.ID_KHACH_HANG = KH.ID_KHACH_HANG AND DA_KH.id_du_an = $id_du_an");

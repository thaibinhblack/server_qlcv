...
baocao_taosl_hoatdongdieutri (p_dvtt,p_tungay,p_denngay,p_cachlay);
   ...
   delete from his_reports.baocao_hoatdongdieutri_tong
    where dvtt = p_dvtt ;

   insert into his_reports.baocao_hoatdongdieutri_tong(khoa,
                                                       sogiuong,
                                                       benhdauky,
                                                       benhdauky_nu,
                                                       benhtrongky,
    ...
    select khoa,sogiuong, benhdauky, case when nu = 0 then 0 else benhdauky end  as benhdauky_nu,
...
from his_reports.baocao_hoatdongdieutri
    where dvtt = p_dvtt
    and trunc(ngaybaocao) between p_tungay and p_denngay;

OPEN cur FOR
SELECT a.ten_phongban khoa, c.so_giuong sogiuong, sum(benhdauky) as benhdauky, sum(benhdauky_nu) as benhdauky_nu, sum(benhtrongky) as benhtrongky,
 sum(benhtrongky_nu) as benhtrongky_nu, sum(treem) as treem, sum(treem_nu) as treem_nu, sum(capcuu) as capcuu, sum(capcuu_nu) as capcuu_nu,
 sum(songaydieutri) as songaydieutri, sum(songaydieutri_nu) as songaydieutri_nu, sum(tuvong) as tuvong, sum(tuvong_nu) as tuvong_nu,
 sum(tuvong_treem) as tuvong_treem, sum(tuvong_treem_nu) as tuvong_treem_nu, sum(tuvong_truoc24gio) as tuvong_truoc24gio,
 sum(tuvong_truoc24gio_nu) as tuvong_truoc24gio_nu, sum(cobaohiem) as cobaohiem, sum(cobaohiem_nu) as cobaohiem_nu,

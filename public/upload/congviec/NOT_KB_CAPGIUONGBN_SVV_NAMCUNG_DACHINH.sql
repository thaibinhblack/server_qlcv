...
BEGIN
    ...
    if v_thamso26001 = 1 then
        if v_thamso26002 = 1 then
            ...
            if (v_kiemtracunggiuong > 0 and p_namcungnguoi = 0) or (v_kiemtracunggiuong > p_songuoinamcung and p_namcungnguoi = 1)  then return '-5'; end if;
        end if;
    end if;
    SELECT COUNT(1) INTO  v_kiemtra_cg
    FROM noitru_loggiuongbenh
    WHERE dvtt = p_dvtt
        AND stt_benhan = p_stt_benhan
        AND stt_dotdieutri = p_stt_dotdieutri
        AND ngayra IS NULL;
    IF v_kiemtra_cg > 0 THEN RETURN '-1';  END IF;
    SELECT trang_thai,bant INTO  v_trang_thai,v_ktrbant
    FROM noitru_benhan
    WHERE dvtt = p_dvtt AND stt_benhan = p_stt_benhan AND sovaovien = p_sovaovien;
    if (p_KCAPGIUONGKHIDATHANHTOANNT=1 and v_ktrbant=1) THEN
        ...
        and stt_dotdieutri=p_stt_dotdieutri
        and tt_lantt=1;
        IF (v_kiemtra_tt>0) THEN RETURN '-10'; END IF;
    ...
    SELECT tt_dotdieutri, cobhyt, sobaohiemyte
    INTO v_tt_dotdieutri,v_cobhyt,v_sothebhyt
    FROM noitru_dotdieutri
    WHERE dvtt = p_dvtt
        AND stt_benhan = p_stt_benhan
        AND stt_dotdieutri = p_stt_dotdieutri
    ...
    if p_thamso82785 = '1' then
        ...
        where dvtt = p_dvtt and sovaovien = p_sovaovien and sovaovien_dt = p_sovaovien_dt;
    end if;
    begin
        select mota_thamso into p_thamso
        from his_fw.dm_thamso_donvi where dvtt = p_dvtt and ma_thamso = 208;
        exception when NO_DATA_FOUND THEN p_thamso:= 0; v_thamso:= 0;
    end;
    if p_thamso != 0 then
    LOOP
    begin
        select g.ngay_ap_dung_truyen, g.ma_thamso_truyen_truoc into p_thamso_ngay, v_thamso
    ...
    if p_thamso != 0 then
        begin
            select g.column_gia_bhyt, g.column_gia_kbhyt into p_thamso_gia_bhyt,p_thamso_gia_kbhyt
            from his_manager.Danhmuc_Giathaydoi g
            where g.ma_thamso_truyen= p_thamso;
            exception when no_data_found then p_thamso := 0; v_thamso := 0;
        end;
    end if;
    if p_ngaytao < p_thamso_ngay then p_thamso := v_thamso; end if; EXIT WHEN p_ngaytao >= p_thamso_ngay or p_thamso = 0;
    END LOOP;
    end if;
    -- END TGGDEV-42897
    IF p_thamso = 0 THEN
        ...
        INTO v_giagiuongbenh,v_ngoaidanhmuc,v_sotienbhkchi
        FROM dm_loaigiuongbenh loai, dm_giuongbenh giuong
    ...
    WHERE
        ...
        AND sovaovien = p_sovaovien
        AND sovaovien_dt = p_sovaovien_dt;
    IF  v_tt_dotdieutri <= 2
    THEN UPDATE noitru_dotdieutri
        SET tt_dotdieutri = 1 --
        WHERE dvtt = p_dvtt
            AND stt_benhan = p_stt_benhan
            AND stt_dotdieutri = p_stt_dotdieutri
    ...
    END IF;
    IF v_trang_thai < 2
    THEN UPDATE noitru_benhan
        SET trang_thai = 2 -- dang di?u tr?
        WHERE dvtt = p_dvtt
            AND stt_benhan = p_stt_benhan
            AND  sovaovien = p_sovaovien;
    END IF;
    UPDATE noitru_logkhoaphong
        SET trang_thai = 1 -- c?p nh?t l?i di?u tr? có giu?ng
        WHERE  dvtt = p_dvtt
            AND stt_benhan = p_stt_benhan
            AND stt_dotdieutri = p_stt_dotdieutri
            AND stt_logkhoaphong = p_stt_logkhoaphong
    ...
    INSERT INTO noitru_loggiuongbenh (
        ...
        p_maphongbenh,
        p_magiuongbenh,
        CASE v_cobhyt   WHEN 1 THEN
            CASE WHEN length(v_sothebhyt) = 15  
                THEN  DECODE( v_ngoaidanhmuc,1,0,1) ELSE 0
            END
            ELSE 0
        END,
        p_sovaovien,
        ...
        p_songuoinamcung,
        CASE v_cobhyt WHEN 1 THEN
            CASE WHEN length(v_sothebhyt) = 15   THEN
            v_ngoaidanhmuc ELSE 1
            END
            ELSE 1
        END,
        v_sotienbhkchi,
